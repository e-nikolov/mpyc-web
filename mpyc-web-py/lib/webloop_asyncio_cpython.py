...
import warnings
import weakref
from asyncio import BaseEventLoop as _BaseEventLoop
...
from . import constants, coroutines, events, exceptions, futures, protocols, sslproto, staggered, tasks, transports, trsock
...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def _asyncgen_firstiter_hook(self, agen):
        if self._asyncgens_shutdown_called:
            warnings.warn(
                f"asynchronous generator {agen!r} was scheduled after loop.shutdown_asyncgens() call", ResourceWarning, source=self
            )
    ...
    async def shutdown_default_executor(self, timeout=None):
        ...
        if thread.is_alive():
            warnings.warn(f"The executor did not finishing joining its threads within {timeout} seconds.", RuntimeWarning, stacklevel=2)
        ...
        if host is not None or port is not None:
            ...
            if happy_eyeballs_delay is None:
                ...
                sock, _, _ = await staggered.staggered_race(
                    (functools.partial(self._connect_sock, exceptions, addrinfo, laddr_infos) for addrinfo in infos),
    ...
    async def _create_connection_transport(
        self, sock, protocol_factory, ssl, server_hostname, server_side=False, ssl_handshake_timeout=None, ssl_shutdown_timeout=None
    ):
        sock.setblocking(False)
    ...
    async def create_datagram_endpoint(
        ...
        *,
        ...
        if self._debug:
            if local_addr:
                logger.info(
                    "Datagram endpoint local_addr=%r remote_addr=%r created: (%r, %r)", local_addr, remote_addr, transport, protocol
                )
            else:
